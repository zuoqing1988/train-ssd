import logging
import mxnet as mx
import classify_core.metric_cls as metric_cls
from mxnet.module.module import Module
from classify_core.loader import ImageLoader
from classify_core.imdb import IMDB
from tools.load_model import load_param

def train_net(sym, prefix, ctx, pretrained, epoch, begin_epoch, end_epoch, imdb, color_mode, batch_size, thread_num,
              frequent=50, initialize=True, base_lr=0.01, lr_epoch = [6,14]):
    logger = logging.getLogger()
    logger.setLevel(logging.INFO)
    flip = True
    train_data = ImageLoader(imdb, batch_size, thread_num, color_mode, flip, shuffle=True, ctx=ctx)
    ...
    eval_metrics.add(metric_cls.Accuracy())
